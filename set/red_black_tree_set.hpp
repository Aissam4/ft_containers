...
namespace ft
{
	template<typename T>
		struct Node
		{
			...
			Node(void ) : right(NULL), left(NULL), parent(NULL), color(RED){};
		};
	...
	class RBTree
	{
		...
		private:
			...
			int				_flag;
		public:
			RBTree(const value_compare &comp = value_compare(), const Allocator_type &alloc = Allocator_type()): _data(NULL), _end(NULL)
			{
				this->_size = 0;
				this->_alloc = alloc;
				this->_comp = comp;
		...
		private:
			...
			void DisplayTree(node_type *root, int space)
			{
				...
				if (root->color == RED)
					std::cout<< "\033[0;31mVal -> ( " << p << " \033[0m )" << std::endl;
				else
					std::cout<< "Val -> ( " << p << " )" << std::endl;
				DisplayTree(root->left, space);
			}

			node_type	*search_in_tree(node_type *root, value_type element)
			{
				node_type *tmp = NULL;
				if (root == NULL || root->_data == element)
					tmp = root;
				else if (root->_data < element)
			...
			node_type	*delete_node(node_type *root, value_type key_)
			{
				if (root == NULL) return (NULL);
				else if (key_ < root->_data)
		...
		public:
			...
			node_type	*search(value_type element)
			{
				node_type *tmp = search_in_tree(this->_data, element);
				return (tmp);
			}

			void	erase(value_type key_)
			{
				this->_size--;
